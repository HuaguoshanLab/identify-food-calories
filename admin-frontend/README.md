...
菜谱管理入口 `/admin/recipes` 紧随营养目录。它管理的是“成品菜何时可被选入餐单”，不是另一套营养表：导入 CSV 时名称必须唯一关联当前合格的营养目录，页面提供预校验、模板、导出及当前页批量启用、停用、软删除。每次写操作都要求原因；营养计算和最终资格始终由后端决定。菜谱模板包含配餐用途、餐内角色、食材标签和分类依据，预览及导出同步显示；已移除旧角色维护入口。

菜谱超过 1000 条时，可通过“选择范围 → 选择本批”按每批最多 1000 条替换当前选择，再执行批量操作；不足 1000 条时保留全选全部。新增或删除后，批次范围按最新列表重新计算。

菜谱支持多个适用餐次，可在“编辑分类”中勾选；CSV 的“餐次”列使用 `午餐|晚餐`，单餐次旧文件仍兼容。字段含义、完整可选值与单菜示例见 [菜谱 CSV 导入填写说明](../docs/recipe-csv-guide.md)。
