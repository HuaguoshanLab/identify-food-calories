...
import { classificationSchema, ingredientTagLabels, purposeLabels, roleLabels, reviewClassifications, type RecipeCandidate } from './api'

const schema = classificationSchema.extend({ evidence: z.string().trim().min(1, '请填写分类依据').max(500), reason: z.string().trim().min(1, '请填写修改原因').max(500) })
export function RecipeClassificationEditDialog({ item, accessToken, onClose, onSuccess, onSecurityError }: Readonly<{
  ...
  const { register, handleSubmit, formState: { isSubmitting, errors } } = useForm<z.infer<typeof schema>>({ resolver: zodResolver(schema), defaultValues: { version: 'recipe-classification.v1', purpose: item.classification?.purpose ?? 'unknown', role: item.classification?.role ?? 'unknown', ingredient_tags: item.classification?.ingredient_tags ?? [], evidence: item.classification?.evidence ?? '', basis: 'admin_review', reason: '' } })
  const request = useRef({ payload: '', key: '' })
  const [error, setError] = useState('')
  const submit = handleSubmit(async ({ reason, ...classification }) => {
    const entries = [{ id: item.id, revision: item.revision, catalog_food_name: item.catalog_food_name, classification }]
