...
表格支持当前页勾选（最多 100 条）及批量审核/发布；筛选、翻页清空选择，审核完成保留选择方便继续发布。居中弹窗先读取逐项服务端版本与影响，再统一原因确认。命令顺序执行，保持独立 If-Match、幂等键与后端审计；成功项不重复提交，未知结果仅在本弹窗以原键/原原因重试。明确冲突需核对状态；关闭/刷新后不会在后台继续执行。401/403 停止队列。已发布、未授权或选择后变更的草稿被阻止，不自动授权或代替审核。
