...
export function CatalogListPage({ accessToken, onSessionExpired }: Readonly<{ accessToken?: string; onSessionExpired: () => void }>) {
  ...
  async function edit(draftId: string) {
    setBusy(true)
    setError('')
    try { setEditor({ draft: await readCatalogDraft(accessToken!, draftId) }) }
  ...
  return <section aria-label="营养目录管理" className="space-y-5 p-4 lg:p-6">
    <form aria-label="目录筛选" className="flex flex-wrap items-end gap-x-6 gap-y-4 rounded-lg border bg-card px-5 py-5" onSubmit={form.handleSubmit((values) => { setFilters(values); setPage(1); setNotice('') })}>
      ...
      <div className="ml-auto flex gap-2"><button className={button} onClick={() => { form.reset(emptyCatalogFilters); setFilters(emptyCatalogFilters); setPage(1); setNotice('') }} type="button">重置</button><button className="inline-flex h-9 items-center gap-2 rounded-md bg-blue-600 px-4 text-sm text-white hover:bg-blue-700" type="submit"><Search aria-hidden size={15} />查询</button></div>
    ...
    <section className="min-w-0 rounded-lg border bg-card" aria-labelledby="catalog-list-title">
      <div className="flex flex-wrap items-center justify-between gap-4 p-5">
        ...
        <div className="flex flex-wrap gap-2"><button className="inline-flex h-9 items-center gap-2 rounded-md bg-blue-600 px-3 text-sm text-white hover:bg-blue-700" onClick={() => setEditor({})} type="button"><Plus aria-hidden size={16} />新增</button><button className={button} onClick={() => setImportOpen(true)} type="button"><Upload aria-hidden size={15} />导入</button><button className={button} disabled={busy} onClick={() => void download()} type="button"><Download aria-hidden size={15} />导出</button><button className={button} disabled={busy} onClick={() => void download(true)} type="button">下载模板</button><button aria-label="刷新目录" className={button} disabled={list.isFetching} onClick={() => void list.refetch()} type="button"><RefreshCw aria-hidden className={list.isFetching ? 'animate-spin motion-reduce:animate-none' : ''} size={16} /></button></div>
      </div>
        ...
        <table aria-busy={list.isFetching} className="w-full whitespace-nowrap text-left text-sm"><caption className="sr-only">营养目录列表，营养值按每 100g 计</caption>
          ...
          <tbody className="divide-y">
            {list.isPending ? <tr><td className="p-12 text-center text-muted-foreground" colSpan={11}>正在加载目录…</td></tr> : list.isError ? <tr><td className="p-12 text-center" colSpan={11}><p role="alert">暂时无法加载目录，请稍后重试。</p><button className={`${button} mt-3`} onClick={() => void list.refetch()} type="button">重试</button></td></tr> : !list.data?.items.length ? <tr><td className="p-14 text-center" colSpan={11}><p className="font-medium">{Object.values(filters).some(Boolean) ? '没有符合条件的目录' : '暂无营养目录'}</p><p className="mt-2 text-xs text-muted-foreground">{Object.values(filters).some(Boolean) ? '调整筛选条件，或点击重置查看全部。' : '点击“新增”添加菜品，或下载模板后批量导入。'}</p></td></tr> : list.data.items.map((draft) => <tr className="hover:bg-muted/20" key={draft.id}>
              <td className="max-w-64 truncate px-4 py-4 font-medium" title={draft.canonical_name}><button className="text-blue-600 hover:underline" disabled={busy} onClick={() => void edit(draft.id)} type="button">{draft.canonical_name}</button></td>
              ...
              <td className="sticky right-0 border-l bg-card px-4 py-4"><div className="flex gap-3 text-xs text-blue-600"><button disabled={busy} onClick={() => void edit(draft.id)} type="button">编辑</button><Link to={`/admin/catalog/${draft.id}/lifecycle`}>审核与发布</Link></div></td>
            </tr>)}
          </tbody>
        </table>
      </div>
      <div className="flex flex-wrap items-center justify-end gap-4 border-t px-5 py-4 text-xs text-muted-foreground">
        ...
        <label className="flex items-center gap-2">每页<select aria-label="每页条数" className="h-8 rounded border bg-card px-2" value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1) }}>{[10, 20, 50, 100].map((size) => <option value={size} key={size}>{size} 条</option>)}</select></label>
        <nav aria-label="目录分页" className="flex items-center gap-2"><button aria-label="上一页" className="rounded border p-1.5 disabled:opacity-30" disabled={page <= 1 || list.isFetching} onClick={() => setPage(page - 1)} type="button"><ChevronLeft size={15} /></button><span className="admin-numeric rounded border border-blue-300 px-2.5 py-1 text-blue-600">{page}</span><span>/ {pages}</span><button aria-label="下一页" className="rounded border p-1.5 disabled:opacity-30" disabled={page >= pages || list.isFetching || list.isError} onClick={() => setPage(page + 1)} type="button"><ChevronRight size={15} /></button></nav>
      </div>
    </section>
    {editor && <CatalogDialog title={editor.draft ? '编辑营养目录' : '新增营养目录'} description="填写每 100g 营养值和来源信息，核对后保存为草稿。" onClose={() => setEditor(undefined)}><CatalogDraftPage accessToken={accessToken} embedded initialDraft={editor.draft} key={editor.draft?.id ?? 'new'} onForbidden={() => setForbidden(true)} onSaved={(draft) => saved(`“${draft.canonical_name}”已保存为草稿，可在列表中继续审核与发布。`)} onSessionExpired={onSessionExpired} /></CatalogDialog>}
