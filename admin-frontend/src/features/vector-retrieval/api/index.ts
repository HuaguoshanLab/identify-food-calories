...
const buildSchema = z.object({
  id: z.string().uuid(), vector_space_id: z.string().uuid(), embedding_model: z.string().min(1), embedding_dimension: z.literal(1024), adapter_version: z.string().min(1), retrieval_version: z.string().min(1), snapshot_hash: z.string().regex(/^[a-f0-9]{64}$/), expected_name_count: z.number().int().nonnegative(), pending_count: z.number().int().nonnegative(), failed_count: z.number().int().nonnegative(), completed_count: z.number().int().nonnegative(), status: z.enum(['pending', 'processing', 'partial_failure', 'ready']), requested_at: z.string().datetime({ offset: true }), is_active: z.boolean(), activation_ready: z.boolean(),
}).strict()
