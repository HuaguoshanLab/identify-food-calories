...
import { activateVectorBuild, backfillCatalogSearchIndex, createVectorBuild, listVectorBuilds, retryVectorBuild, type VectorBuild, VectorRetrievalApiError, vectorBuildIdentity } from './api'
...
const labels = { empty: '空构建', pending: '等待处理', processing: '处理中', partial_failure: '部分失败', ready: '已完成' } as const
type Operation = { kind: 'backfill' } | { kind: 'create' } | { kind: 'retry' | 'activate', build: VectorBuild }
...
export function VectorRetrievalPage() {
  ...
  async function submit() {
    ...
    try {
      if (operation.kind === 'backfill') {
        const result = await backfillCatalogSearchIndex(accessToken, { reason: reason.trim() }, crypto.randomUUID())
        setNotice(`检索名称回填完成：更新 ${result.publication_count} 条目录，创建 ${result.name_count} 个名称，排队 ${result.embedding_job_count} 个任务。现在可以创建新的向量构建。`)
      ...
      if (!handleSecurityError(cause)) setError(cause instanceof VectorRetrievalApiError && cause.status === 409 ? operation.kind === 'create' ? '没有可构建的检索名称，或目录状态已变化。请先回填检索名称，再创建新构建。' : '服务端安全校验未通过或数据已变化，请刷新后核对。' : '操作结果未确认，请刷新记录后再试。')
  ...
  return <main aria-label="向量检索管理" className="space-y-5 p-4 lg:p-6">
    ...
    <section className="rounded-lg border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-xl font-semibold">向量检索管理</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">先回填当前合格目录的检索名称，再创建冻结名称快照并排队。激活必须由后端重新验证完成证据及冻结评测发布，页面不会自行放行。</p></div><div className="flex flex-wrap gap-2"><button className={button} disabled={busy || query.isFetching} onClick={() => { setReason(''); setOperation({ kind: 'backfill' }) }} type="button">回填检索名称</button><button className={primary} disabled={busy || query.isFetching} onClick={() => { setReason(''); setOperation({ kind: 'create' }) }} type="button">创建 DashScope 构建</button></div></div><p className="mt-4 rounded border bg-muted/30 p-3 text-xs text-muted-foreground">本次构建身份：{vectorBuildIdentity.embedding_model} · {vectorBuildIdentity.embedding_dimension} 维 · {vectorBuildIdentity.adapter_version} · {vectorBuildIdentity.retrieval_version}</p></section>
    {query.data?.items.some(build => build.status === 'empty') ? <p className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900" role="status">检测到历史空构建。它没有处理任何目录，也不能激活。请先回填检索名称，再创建新的构建。</p> : null}
