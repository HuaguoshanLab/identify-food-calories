...
describe('AdminRouteGuard', () => {
  ...
  it('401、无 token、网络失败或无效 probe DTO 都清空 cache 并 fail closed 到 login，且不调用持久化 API', async () => {
    ...
    const second = render(<GuardFixture establishSession={false} queryClient={noTokenCache} />)
    expect(await screen.findByRole('heading', { name: '后台登录' })).toBeVisible()
    await waitFor(() => expect(noTokenCache.getQueryData(['admin', 'private'])).toBeUndefined())
    expect(storageSetItem).not.toHaveBeenCalled()
    second.unmount()

    const networkCache = new QueryClient()
    networkCache.setQueryData(['admin', 'private'], { title: '不应残留' })
    vi.spyOn(globalThis, 'fetch').mockRejectedValueOnce(new Error('network unavailable'))
    render(<GuardFixture queryClient={networkCache} />)
    expect(await screen.findByRole('heading', { name: '后台登录' })).toBeVisible()
    await waitFor(() => expect(networkCache.getQueryData(['admin', 'private'])).toBeUndefined())
    expect(storageSetItem).not.toHaveBeenCalled()
  })
