...
在 `backend/` 执行 `uv run alembic upgrade head`，再重启后端并更新后台。0032 将候选的 `meal_slot` 转为 `meal_slots` 数组，旧值原样转换为单元素数组。后台编辑支持多选，CSV 餐次使用 `午餐|晚餐`，旧单餐次文件仍兼容；筛选和规划按数组包含关系查询，同一条候选无需重复导入。

不要让旧后端与新 schema 混用。降级前必须通过正常编辑将所有候选收敛为一个餐次；存在多餐次记录时迁移会拒绝降级，避免静默丢失配置。历史餐单快照不变。
