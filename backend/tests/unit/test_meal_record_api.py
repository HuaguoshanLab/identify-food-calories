...
def _record(*, user_id: uuid.UUID) -> MealRecord:
    return MealRecord(
        ...
        protein_g=Decimal("2.4"),
        fat_g=Decimal("0.3"),
        carbohydrate_g=Decimal("28.2"),
        created_at=NOW,
        updated_at=NOW,
        deleted_at=None,
        items=[
...
class StubMealRecordService:
    ...
    def confirm_dashboard_time_zone(self, *, user_id: uuid.UUID, time_zone: str):
        ...
        return type(
            "Confirmation", (), {"dashboard_time_zone": time_zone, "confirmed_at": NOW}
        )()
...
def test_meal_record_openapi_and_schema_reject_client_nutrition_values() -> None:
    ...
    with _client(principal=user_id, service=service) as client:
        ...
        assert set(openapi["paths"]["/api/v1/meal-records/{record_id}"]) == {
            "get",
            "patch",
            "delete",
        }
...
def test_authenticated_owner_can_list_and_confirm_without_exposing_agent_run_ids() -> (
    None
):
    user_id = uuid.uuid4()
    ...
    assert (
        ...
        and "agent_run_id" not in confirmed.json()
        and "source_run_id" not in confirmed.json()
    )
    assert service.confirm_calls[0][0] == user_id


def test_time_zone_contract_requires_iana_input_and_exposes_only_current_statistical_basis() -> (
    None
):
    user_id = uuid.uuid4()
    service = StubMealRecordService(_record(user_id=user_id))
    with _client(principal=user_id, service=service) as client:
        ...
        saved_without_zone = client.post(
            "/api/v1/meal-records",
            json={"thread_id": str(uuid.uuid4()), "command_key": "save-key-00000002"},
        )
...
def test_dashboard_timezone_conflict_is_generic_and_does_not_disclose_stored_basis() -> (
    None
):
    user_id = uuid.uuid4()
    service = StubMealRecordService(_record(user_id=user_id))
...
def test_records_write_routes_map_invalid_timezone_to_safe_400_without_details(
...
def test_foreign_record_uuid_has_the_same_not_found_result_for_get_patch_and_delete() -> (
    None
):
    ...
    with _client(principal=other, service=service) as client:
        ...
        assert (
            client.patch(
                f"/api/v1/meal-records/{record.id}",
                json={"consumed_at": NOW.isoformat(), "time_zone": "UTC"},
            ).status_code
            == 404
        )
