...
| `test_catalog_embedding_jobs.py` | 发布时的 canonical/alias × active vector space job 入队、仅安全字段的聚合状态、publication 级失败重试、immutable vector-space build 的快照/重放/不激活，以及 worker 的 SKIP LOCKED 租约、Provider 最小输入、向量写回和 exact completion evidence 真实 PostgreSQL 证据。 |
