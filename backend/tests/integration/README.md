...
| `test_agent_bootstrap.py` | 真实 PostgreSQL 的 migration → checkpoint setup → FDC seed 幂等初始化链，以及 lifespan 受监督 embedding worker 的启动/停止与禁用降级证据。 |
...
| `test_hybrid_food_search.py` | 真实 PostgreSQL 检查 schema、`pg_trgm`/cosine 三通道、当前资格与确认 reread、1024 维隔离 vector space、受控关系撤销、有限 job、build/activation hash evidence，以及冻结 snapshot evaluator 的真实 SQL/release evidence。 |
| `test_catalog_embedding_jobs.py` | 发布时的 canonical/alias × active vector space job 入队、仅安全字段的聚合状态、publication 级失败重试、immutable vector-space build 的快照/重放/不激活，以及 worker 的 SKIP LOCKED 租约、Provider 最小输入、向量写回、exact completion evidence 与 release/hash 绑定的原子激活真实 PostgreSQL 证据。 |
