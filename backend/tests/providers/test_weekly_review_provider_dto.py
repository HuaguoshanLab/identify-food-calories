...
from app.providers.reasoning.dto import ProviderUsageDTO, WeeklyReviewOutputDTO, WeeklyReviewRequest, WeeklyReviewSuggestionDTO
...
def test_weekly_request_has_only_deidentified_allowlisted_facts() -> None:
    ...
    with pytest.raises(ValidationError):
        WeeklyReviewRequest.model_validate({**request.model_dump(), "facts": {**request.facts, "email": "x@example.invalid"}})
...
def test_weekly_output_rejects_numeric_medical_coercive_and_reasoning_text(text: str) -> None:
    with pytest.raises(ValidationError):
        WeeklyReviewOutputDTO(suggestions=[WeeklyReviewSuggestionDTO(category="food_variety", text=text)], disclaimer="仅基于已记录数据，供一般饮食参考，不构成医疗建议。")


@pytest.mark.asyncio
async def test_fake_discards_weekly_request_body_from_trace() -> None:
    ...
    await provider.generate_weekly_review(WeeklyReviewRequest(facts=_facts()))
