...
| `state.py` | 版本化、受限、JSON-safe 的 MealAgentState 与独立 DietPlanningState；非精确餐食/规划确认只保存 ID、目录版本和允许的说明投影，恢复时必须权威复读。 |
...
| `graph.py` | 主图路由、受限餐食/规划子图、同构的候选 ASK/resume 语义、槽位局部替换和 FastAPI lifespan runtime 合同。 |
...
- 完成的饮食规划通过 planning 的 `PlanArchiveWriter` 与独立 `PlanArchiveWrite` DTO 写正式存档；AgentService 统一提交运行完成状态与存档。planning Repository 只读本模块线程/事件是否可恢复，不把临时存储当作长期餐单权威。
