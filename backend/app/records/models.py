...
class MealRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "energy_kcal >= 0 AND protein_g >= 0 AND fat_g >= 0 AND carbohydrate_g >= 0",
            name="ck_meal_records_nonnegative_totals",
        ),
        CheckConstraint(
            "meal_slot IS NULL OR meal_slot IN ('breakfast', 'lunch', 'dinner', 'snack')",
            name="ck_meal_records_meal_slot",
        ),
        CheckConstraint(
            "consumed_at <= updated_at", name="ck_meal_records_consumed_before_update"
        ),
        UniqueConstraint(
            "user_id", "source_run_id", name="uq_meal_records_user_source_run"
        ),
        UniqueConstraint(
            "user_id", "command_key", name="uq_meal_records_user_command_key"
        ),
        Index(
            "ix_meal_records_user_consumed_active",
            "user_id",
            "consumed_at",
            "id",
        ...
        Index(
            "ix_meal_records_user_local_date_active",
            "user_id",
            "consumed_local_date",
            "id",
            postgresql_where=text("deleted_at IS NULL"),
    ...
    source_run_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("agent_runs.id", ondelete="RESTRICT"), nullable=False
    )
    agent_thread_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("agent_threads.id", ondelete="RESTRICT"), nullable=False
    )
...
class DashboardTimezoneBackfillAudit(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_dashboard_timezone_backfill_audits_count",
        ),
    )
...
class MealRecordItem(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "position >= 0", name="ck_meal_record_items_position_nonnegative"
        ),
        CheckConstraint("grams > 0", name="ck_meal_record_items_grams_positive"),
        CheckConstraint(
            "energy_kcal >= 0 AND protein_g >= 0 AND fat_g >= 0 AND carbohydrate_g >= 0",
            name="ck_meal_record_items_nonnegative_nutrients",
        ),
        UniqueConstraint(
            "record_id", "position", name="uq_meal_record_items_record_position"
        ),
    ...
    record_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("meal_records.id", ondelete="CASCADE"), nullable=False
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("users.id", ondelete="CASCADE"), nullable=False
    )
...
class PreferenceMemoryLedger(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "category IN ('goal', 'avoidance', 'stable_preference')",
            name="ck_preference_memory_ledger_category",
        ),
        CheckConstraint(
            "source_kind IN ('user_statement', 'model_inference', 'user_maintained')",
            name="ck_preference_memory_ledger_source_kind",
        ),
        CheckConstraint(
            "canonical_text = btrim(canonical_text) AND canonical_text <> ''",
            name="ck_preference_memory_ledger_canonical_text",
        ),
        CheckConstraint(
            "provisioning_status IN ('pending', 'claimed', 'provisioned', 'outcome_unknown', 'failed', 'cancelled')",
            name="ck_preference_memory_ledger_provisioning_status",
        ),
        Index(
            "ix_preference_memory_ledger_user_active",
            "user_id",
            "category",
            "updated_at",
...
class MemoryDeletionOutbox(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "operation IN ('delete_external_memory')",
            name="ck_memory_deletion_outbox_operation",
        ),
        CheckConstraint(
            "status IN ('pending', 'processing', 'completed', 'failed')",
            name="ck_memory_deletion_outbox_status",
        ),
        CheckConstraint("attempt >= 0", name="ck_memory_deletion_outbox_attempt"),
        UniqueConstraint(
            "ledger_id", "operation", name="uq_memory_deletion_outbox_ledger_operation"
        ),
    ...
    ledger_id: Mapped[uuid.UUID] = mapped_column(
        Uuid,
        ForeignKey("preference_memory_ledger.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class MemoryProvisionOutbox(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "operation IN ('provision_external_memory')",
            name="ck_memory_provision_outbox_operation",
        ),
        CheckConstraint(
        ...
        UniqueConstraint(
            "ledger_id", "operation", name="uq_memory_provision_outbox_ledger_operation"
        ),
