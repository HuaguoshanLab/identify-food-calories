...
- Domain Service 只通过 `PlanningRepository` 和 `PlanningNutritionPort` Protocol 获取资料、受控菜谱/成品菜候选和营养数据；每个菜品必须调用 nutrition 的确定性 `calculate_nutrition`，不得读取或保存 recipe total。候选可引用系统目录或管理员已发布目录，二者都只保存引用与份量，不复制营养。
...
| `repository.py` | SQLAlchemy tenant-filtered profile/projection adapter、已归档候选轮换历史，以及排除失格或停用目录项的 recipe/candidate future query |
| `service.py` | `target-policy.v1`、健康 guard、目录重算的餐单组合、最近三份历史优先去重的候选轮换、确定性校验、completion projection 和 profile 事务边界 |
