...
| `search_repository.py` | exact、pg_trgm 与 pgvector 三通道的 PostgreSQL adapter；复用当前 publication 资格 SQL、按冻结 manifest 独立 reconcile completion evidence，并在确认时重新读取权威记录 |
| `index_worker.py` | 由 FastAPI lifespan supervisor 持有的有限租约 worker；可按 immutable build ID 限定消费其 manifest 中的任务，在 Provider I/O 前后重读资格，安全写回向量；空闲 scoped worker 只重算同一 build/空间的 manifest evidence，绝不激活空间或重复调用 Provider。 |
