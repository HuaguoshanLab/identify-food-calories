...
| `index_worker.py` | 由 FastAPI lifespan supervisor 持有的有限租约 worker；只消费 immutable vector-space build manifest 中的任务，在 Provider I/O 前后重读资格，安全写回向量；空闲 scoped worker 只重算已有空间的 manifest evidence，绝不激活空间或重复调用 Provider。 |
