...
export function AnalyzePage() {
  ...
  return (
    <section className="mx-auto w-full max-w-xl space-y-4 pb-4">
      <div className="px-1 pb-2"><p className="mb-2 text-xs font-semibold tracking-[0.18em] text-primary">每一餐，都值得好好对待</p><h2 className="text-[30px] font-semibold leading-[1.3] tracking-tight">今天，吃了什么？</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">从一餐开始，慢慢了解自己的饮食。</p></div>
      <Card className="border-0 shadow-sm"><CardHeader><CardTitle>{inputMode === 'image' ? '上传餐食图片' : '文字描述餐食'}</CardTitle><CardDescription>{inputMode === 'image' ? '选择一张餐食图片，系统会识别菜品并在需要时向你确认。' : '用文字描述这一餐吃了什么，越详细估算越准确。'}</CardDescription></CardHeader><CardContent className="space-y-3">
        {inputMode === 'image' ? <>
