  ...
  return (
    <section className="mx-auto w-full max-w-xl space-y-4 pb-4">
      ...
      {snapshot?.status === 'completed' && report?.totals && canDisplayReport ? <div className="space-y-4">
        <Card aria-label="估算总热量"><CardContent className="p-4"><p className="text-base font-medium text-muted-foreground">{report.is_partial ? '已计入项目的估算总热量' : '估算总热量'}</p><p className="mt-1 flex items-baseline gap-1 tabular-nums"><span className="text-4xl font-bold leading-10 text-foreground">{report.totals.energy_kcal}</span><span className="text-sm font-medium text-muted-foreground">kcal</span></p><p className="mt-2 text-base leading-6 text-muted-foreground">数值由受控营养目录计算，实际份量可能有偏差。</p></CardContent></Card>
        <Card aria-label="食物明细"><CardHeader className="pb-2"><h2 className="text-base font-semibold leading-6">食物明细</h2></CardHeader><CardContent className="space-y-2">{report.items?.map((item) => <article className="space-y-1 rounded-lg border border-border p-3" key={item.item_id ?? `${item.name}-${item.grams}`}><div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-semibold leading-6">{displayFoodName(item.name)}</h3>{item.is_estimated ? <Badge variant="outline">估算重量</Badge> : null}</div><p className="tabular-nums text-base leading-6">{item.grams}g · {item.energy_kcal} kcal</p>{item.is_estimated ? <p className="text-[13px] leading-5 text-muted-foreground">估算重量，可能与实际份量存在偏差。</p> : null}<p className="tabular-nums text-sm leading-5 text-muted-foreground">蛋白质 {item.protein_g}g · 脂肪 {item.fat_g}g · 碳水 {item.carbohydrate_g}g</p></article>)}<p className="pt-1 text-[13px] leading-5 text-muted-foreground">份量为估算值，可在下方修正</p></CardContent></Card>
        <Card aria-label="营养汇总"><CardContent className="space-y-3 p-4"><p className="tabular-nums text-base leading-6 text-muted-foreground">蛋白质 {report.totals.protein_g}g · 脂肪 {report.totals.fat_g}g · 碳水 {report.totals.carbohydrate_g}g</p>{report.context_references?.map((reference) => <p className="text-sm leading-5 text-muted-foreground" key={reference}>{reference}</p>)}<p className="text-sm leading-5 text-muted-foreground">{report.disclaimer || '本结果仅供一般饮食参考，不替代医疗建议。'}</p></CardContent></Card>
        {!report.is_partial ? <Card aria-label="保存餐食记录"><CardContent className="p-4">{savedRecordId ? <div className="space-y-2"><p className="text-sm font-medium text-primary">已保存</p><Link className="inline-flex h-11 w-full items-center justify-center rounded-lg border border-input text-sm font-medium" to={`/app/records/${savedRecordId}`}>查看记录</Link></div> : <form className="space-y-3" onSubmit={(event) => void saveForm.handleSubmit(confirmSave)(event)} noValidate><fieldset className="space-y-3" disabled={savingRecord}><div className="space-y-1"><Label className="text-[13px]" htmlFor="save-meal-slot">餐次</Label><select className="h-11 w-full rounded-lg border border-input bg-background px-3 text-base" id="save-meal-slot" {...saveForm.register('mealSlot')}>{Object.entries(mealSlotLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><div className="space-y-1"><Label className="text-[13px]" htmlFor="save-consumed-at">用餐时间</Label><Input className="h-11" id="save-consumed-at" type="datetime-local" max={localMealTime()} {...saveForm.register('consumedAt')} aria-invalid={Boolean(saveForm.formState.errors.consumedAt)} aria-describedby="save-time-help" /><p className="text-[13px] text-muted-foreground" id="save-time-help">按当前设备时区填写；补录时请确认餐次和时间。</p></div>{saveForm.formState.errors.consumedAt ? <p className="text-sm text-destructive" role="alert">{saveForm.formState.errors.consumedAt.message}</p> : null}<Button className="h-11 w-full" disabled={savingRecord} type="submit">{savingRecord ? '正在保存…' : '确认并保存'}</Button></fieldset>{saveError ? <p className="mt-2 text-sm text-destructive" role="alert">{saveError}</p> : null}</form>}</CardContent></Card> : null}
        <Card aria-label="修正项目区域"><CardContent className="space-y-2 p-4">{followupError ? <p className="text-sm text-destructive" role="alert">{followupError}</p> : null}<Label className="text-[13px]" htmlFor="meal-correction">修正或排除项目</Label><div className="flex gap-2"><Input className="h-11 flex-1" id="meal-correction" onChange={(event) => setCorrection(event.target.value)} placeholder="例如：米饭改为 150 克，或排除米饭" value={correction} /><Button className="h-11 shrink-0 px-4" disabled={isBusy} onClick={submitCorrection} type="button" variant="outline">应用修正</Button></div></CardContent></Card>
        <Card aria-label="删除这次分析"><CardContent className="space-y-2 p-4"><p className="text-sm font-medium">删除这次分析</p><p className="text-[13px] leading-5 text-muted-foreground">提交后立即关闭此会话的分析、事件流和本地缓存；数据将在 24 小时内删除。</p><Button className="min-h-11 w-full" disabled={status === 'deleting'} onClick={() => { setDeleteError(''); setDeleteOpen(true) }} type="button" variant="destructive">删除这次分析</Button></CardContent></Card>
      </div> : null}
