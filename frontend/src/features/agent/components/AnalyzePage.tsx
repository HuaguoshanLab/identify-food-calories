...
import { Camera, ChevronDown, ChevronRight, ChevronUp, CircleAlert, CircleCheck, ImagePlus, MessageSquareText, RefreshCw, ShieldCheck, Trash2, X } from 'lucide-react'
...
export function AnalyzePage() {
  ...
  return (
    <section className="mx-auto w-full max-w-xl space-y-4 pb-4">
      <Card className="border-0 shadow-sm"><CardHeader><CardTitle>{inputMode === 'image' ? '上传餐食图片' : '文字描述餐食'}</CardTitle><CardDescription>{inputMode === 'image' ? '拍下整餐，尽量看清食物。' : '写明食物和份量，例如：米饭 100 克。'}</CardDescription></CardHeader><CardContent className="space-y-3">
        {inputMode === 'image' ? <>
          {imagePreviewUrl ? <div className="relative overflow-hidden rounded-lg border border-border"><img alt="已选择的餐食图片" className="aspect-[4/3] w-full object-cover" src={imagePreviewUrl} /><Button aria-label="移除已选图片" className="absolute right-2 top-2 size-8 rounded-full bg-background/90 p-0 shadow-sm hover:bg-background" disabled={isBusy} onClick={clearSelectedImage} type="button" variant="secondary"><X aria-hidden="true" className="size-4" /></Button></div> : <div className="rounded-2xl bg-accent/50 p-4"><div aria-hidden="true" className="mx-auto mb-5 flex size-24 items-center justify-center rounded-full border border-primary/15 bg-card/70"><Camera className="size-10 text-primary" strokeWidth={1.3} /></div><div className="grid grid-cols-2 gap-2"><Button className="h-12" disabled={isBusy} onClick={() => cameraInputRef.current?.click()} type="button"><Camera aria-hidden="true" className="size-5" />拍照</Button><Button className="h-12 bg-card" disabled={isBusy} onClick={() => galleryInputRef.current?.click()} type="button" variant="outline"><ImagePlus aria-hidden="true" className="size-5" />从相册选择</Button></div></div>}
          ...
          {!imagePreviewUrl ? <p id="meal-image-help" className="text-[13px] leading-5 text-muted-foreground">支持 JPG、PNG、WebP，最大 10 MB。</p> : null}
      ...
      {(snapshot?.status === 'completed' || correctionPending) && report?.totals && canDisplayReport ? <div className="space-y-4">
        <Card aria-label="估算总热量" className="gap-3 border-primary/20 bg-accent py-5 [--card-spacing:--spacing(5)]"><CardContent className="px-4"><p className="text-sm font-medium text-muted-foreground">{report.is_partial ? '已计入项目的估算总热量' : '估算总热量'}</p><p className="mt-0.5 flex items-baseline gap-1 tabular-nums"><span className="text-3xl font-bold leading-9 text-foreground">{report.totals.energy_kcal}</span><span className="text-[13px] font-medium text-muted-foreground">kcal</span></p><p className="mt-1 text-[13px] leading-5 text-muted-foreground">数值由受控营养目录计算，实际份量可能有偏差。</p></CardContent></Card>
