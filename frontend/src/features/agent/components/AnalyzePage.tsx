...
import { Camera, ChevronDown, ChevronUp, CircleAlert, CircleCheck, ImagePlus, MessageSquareText, RefreshCw, ShieldCheck, Sparkles, X } from 'lucide-react'
...
function MacroComposition({ totals }: { totals: Record<string, string> }) {
  ...
  return <Card aria-label="营养素构成"><CardHeader className="pb-2"><h2 className="text-base font-semibold leading-6">营养素构成</h2></CardHeader><CardContent className="space-y-4"><div className="relative mx-auto size-48" role="img" aria-label={`总热量 ${energy} kcal；营养素估算供能占比：${segments.map((segment) => `${segment.label} ${segment.share.toFixed(1)}%`).join('，')}`}><svg aria-hidden="true" className="size-full -rotate-90" viewBox="0 0 200 200"><circle cx="100" cy="100" fill="none" r="72" stroke="var(--muted)" strokeWidth="26" />{segments.filter((segment) => segment.share > 0).map((segment) => <circle cx="100" cy="100" fill="none" key={segment.key} pathLength="100" r="72" stroke={segment.color} strokeDasharray={`${Math.max(0, segment.share - Math.min(1.2, segment.share / 2))} 100`} strokeDashoffset={-segment.offset} strokeWidth="26" />)}</svg><div aria-hidden="true" className="absolute inset-0 flex flex-col items-center justify-center gap-1"><span className="text-sm font-medium text-muted-foreground">总热量</span><span className="tabular-nums text-2xl font-bold leading-7">{energy}</span><span className="text-sm text-muted-foreground">kcal</span></div></div><dl className="grid grid-cols-3 gap-2 text-center">{segments.map((segment) => <div key={segment.key}><dt className="flex items-center justify-center gap-1 text-sm font-semibold"><span aria-hidden="true" className="size-2.5 rounded-full" style={{ backgroundColor: segment.color }} />{segment.label}</dt><dd className="mt-1 tabular-nums text-sm font-semibold text-muted-foreground">{segment.value.toFixed(1)}g</dd></div>)}</dl></CardContent></Card>
}
  ...
  return (
    <section className="mx-auto w-full max-w-xl space-y-4 pb-4">
      ...
      {snapshot?.status === 'completed' && report?.totals && canDisplayReport ? <div className="space-y-4">
        ...
        <Card aria-label="营养说明"><CardContent className="space-y-3 p-4">{report.context_references?.map((reference) => <p className="flex items-center gap-2 text-sm leading-5 text-muted-foreground" key={reference}><Sparkles aria-hidden="true" className="size-4 shrink-0 text-primary" />{reference}</p>)}<p className="text-sm leading-5 text-muted-foreground">{report.disclaimer || '本结果仅供一般饮食参考，不替代医疗建议。'}</p></CardContent></Card>
