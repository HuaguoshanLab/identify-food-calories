...
describe('AnalyzePage', () => {
  ...
  it('explains empty and too-long descriptions next to the input', async () => {
    ...
    await user.click(screen.getByLabelText('餐食描述'))
...
it('确认保存提交明确早餐与补录时间，重复点击不会重复提交', async () => {
  ...
  await user.type(screen.getByLabelText('餐食描述'), '米饭 100 克')
  await user.click(screen.getByRole('button', { name: '开始分析' }))
  await screen.findByRole('radiogroup', { name: '餐次' })
  await user.click(screen.getByRole('radio', { name: '早餐' }))
  ...
  expect(screen.getByRole('radio', { name: '早餐' })).toHaveAttribute('aria-checked', 'true')
})
