...
- 为完成“分析完成后确认保存”这一用户动作，`components/AnalyzePage.tsx` 可以调用 `features/records/api/client.ts` 的公开 `confirmMealRecord` 及其导出的餐次/时间表单合同；不得导入 records 的组件、schema 私有实现或状态。
