...
describe('saved plans', () => {
  ...
  it('reads older versions and requires confirmation before deleting all versions', async () => {
    ...
    expect(screen.queryByText('保存的计划')).not.toBeInTheDocument()
    expect(screen.queryByText(/历史版本仅供查看/)).not.toBeInTheDocument()
    expect(screen.queryByRole('link', { name: '返回历史计划' })).not.toBeInTheDocument()
