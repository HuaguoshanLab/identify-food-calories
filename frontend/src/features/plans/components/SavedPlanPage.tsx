...
export function SavedPlanPage() {
  ...
  return <section className="space-y-4">
    <h2 className="text-xl font-semibold">保存的计划</h2>
    ...
    {plan ? <>
      <p className="tabular-nums">{plan.plan_date} · 第 {plan.version} / {plan.current_version} 版</p>
      <p className="text-sm text-muted-foreground">已保存 · {plan.time_zone} · 历史版本仅供查看</p>
      <div className="flex gap-2"><Button className="h-11" disabled={plan.version <= 1} onClick={() => setParams({ id, version: String(plan.version - 1) })} variant="outline">上一版</Button><Button className="h-11" disabled={plan.version >= plan.current_version} onClick={() => setParams({ id, version: String(plan.version + 1) })} variant="outline">下一版</Button></div>
      <PlanOverview report={plan.report} />
      {relaxation ? <Alert><AlertTitle>此版本已放宽部分营养目标</AlertTitle><p>{relaxation.reason}</p></Alert> : null}
      <section aria-label="保存的三餐" className="space-y-3"><h2 className="text-xl font-semibold">三餐内容</h2>{plan.report.meals.map((meal) => <MealCard key={meal.slot} meal={meal} />)}</section>
      <p className="text-sm text-muted-foreground">{plan.report.disclaimer} 计划不计入实际摄入。</p>
      <Button className="h-11" onClick={() => setConfirming(true)} variant="destructive">删除这天的计划</Button>
