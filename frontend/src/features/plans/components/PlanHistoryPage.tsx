...
export function PlanHistoryPage() {
  ...
  return <section className="space-y-4">
    ...
    <div className="space-y-3">{history.data?.pages.flatMap((page) => page.items).map((plan) => <Link className="flex items-center gap-3 rounded-xl border border-border bg-card p-4 shadow-sm transition-colors hover:bg-muted focus-visible:ring-3 focus-visible:ring-ring/50" key={plan.id} to={`${routePaths.planDetail}?id=${plan.id}`}><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-muted text-primary"><CalendarDays aria-hidden="true" className="size-5" /></span><p className="min-w-0 flex-1 font-semibold tabular-nums">{plan.plan_date} 三餐计划</p><ChevronRight aria-hidden="true" className="size-4 shrink-0 text-muted-foreground" /></Link>)}</div>
