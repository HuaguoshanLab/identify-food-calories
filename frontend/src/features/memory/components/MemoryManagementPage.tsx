...
export function MemoryManagementPage() { const { request } = useAuth(); const [memories, setMemories] = useState<Memory[]>(); const [error, setError] = useState(''); useEffect(() => { void listMemories(request).then(setMemories).catch(() => setError('暂时无法加载记忆。')) }, [request]); return <section className="space-y-4"><div><p className="text-[15px] text-muted-foreground">查看和管理长期饮食偏好。明确写了“今天”或“本次”的临时要求不会自动保存到这里。</p></div>{error ? <p className="text-sm text-destructive">{error}</p> : null}{memories === undefined && !error ? <p className="text-sm text-muted-foreground">正在加载…</p> : null}{memories?.length === 0 ? <Card><CardContent className="py-8 text-center text-sm text-muted-foreground">还没有长期偏好。你在分析时明确说明的饮食目标和忌口会保存在这里。</CardContent></Card> : null}{memories?.map((memory) => <Link className="block rounded-xl transition-colors hover:bg-muted" key={memory.id} to={`/app/me/memories/${memory.id}/edit`}><Card><CardContent className="space-y-2"><p className="break-words text-[15px] font-semibold leading-6">{label[memory.category]} · {memory.canonical_text}</p><p className="text-[13px] leading-5 text-muted-foreground tabular-nums">{source[memory.source_kind]} · 更新于 {new Date(memory.updated_at).toLocaleString('zh-CN')}</p></CardContent></Card></Link>)}</section> }
