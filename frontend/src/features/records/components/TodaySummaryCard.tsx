...
export function TodaySummaryCard({ overview }: { overview: SummaryOverview }) {
  ...
  return <Card aria-labelledby="today-summary-title"><CardHeader><CardTitle id="today-summary-title">今日已记录摄入</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex items-end justify-between gap-4"><p className="tabular-nums text-3xl font-bold leading-none">{formatEnergy(today.totals.energy_kcal)} kcal</p><p className="tabular-nums text-sm text-muted-foreground">{today.meal_count} 餐</p></div>{macros ? <ul aria-label="宏量营养素相对目标状态" className="grid grid-cols-3 gap-2 text-center text-sm">{macros.map(([label, value, range]) => <li className="rounded-lg bg-muted px-2 py-2" key={label}><span className="block text-muted-foreground">{label}</span><strong className="tabular-nums">{macroState(value, range)}</strong></li>)}</ul> : <p className="text-sm text-muted-foreground">尚未获得可用的营养目标</p>}</CardContent></Card>
}
