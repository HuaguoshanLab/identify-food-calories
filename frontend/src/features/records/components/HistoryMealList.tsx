...
export function HistoryMealList({ isLoadingMore, onLoadMore, page }: { isLoadingMore: boolean; onLoadMore: (cursor: string) => void; page: DashboardHistoryPage }) { return <section aria-labelledby="history-title" className="space-y-4 rounded-xl border border-border bg-card p-4 shadow-sm"><h2 className="text-base font-semibold leading-6" id="history-title">历史记录</h2>{page.groups.length === 0 ? <Card><CardContent className="py-8 text-center"><p className="font-medium">还没有已保存的餐食</p><p className="mt-1 text-sm text-muted-foreground">完成一餐分析后，确认保存即可在这里查看。</p></CardContent></Card> : page.groups.map((group) => <div className="space-y-2" key={group.consumed_local_date}><h3 className="text-sm font-semibold text-muted-foreground">{dateFormatter.format(new Date(`${group.consumed_local_date}T00:00:00`))}</h3>{group.items.map((record) => <Link className="block rounded-md focus-visible:outline-none" key={record.id} to={`/app/records/${record.id}`}><Card className="rounded-md py-3 shadow-none transition-colors hover:bg-muted/40"><CardContent className="flex items-center gap-2 px-2.5 min-[375px]:gap-3">
  <span aria-hidden="true" className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-sm font-semibold text-foreground">{record.meal_slot ? mealSlotLabels[record.meal_slot][0] : '餐'}</span>
