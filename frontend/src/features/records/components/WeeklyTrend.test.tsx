...
const week = Array.from({ length: 7 }, (_, index) => ({
  consumed_local_date: `2026-09-${String(1 + index).padStart(2, '0')}`,
...
describe('WeeklyTrend', () => {
  it('为固定七日 SVG 提供图像语义、键盘点与相同数据表', () => {
    ...
    const point = screen.getByRole('button', { name: /9月1日.*100 kcal/ })
    point.focus()
    fireEvent.keyDown(point, { key: 'ArrowRight' })
    expect(screen.getByRole('button', { name: /9月2日.*200 kcal/ })).toHaveFocus()
  })
})
