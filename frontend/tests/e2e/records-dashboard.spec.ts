...
test('admin RuntimeConfig preflight enables a normal user analyze, safe SSE, save, and Records dashboard', async ({ browser, page, request }) => {
  ...
  for (const timeZone of timezoneContexts) {
    ...
    try {
      ...
      await userPage.getByLabel('餐次', { exact: true }).selectOption('breakfast')
      ...
      await expect(userPage.getByRole('link', { name: /早餐 ·/ })).toBeVisible()
      await userPage.getByRole('link', { name: /早餐 ·/ }).click()
      await userPage.getByRole('link', { name: '编辑餐食记录' }).click()
      await expect(userPage.getByLabel('餐次', { exact: true })).toHaveValue('breakfast')
      await userPage.setViewportSize({ width: 320, height: 740 })
      expect(await userPage.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
      await expect(userPage.getByRole('button', { name: '保存修改' })).toBeVisible()
      await userPage.getByLabel('用餐时间', { exact: true }).fill('2026-01-01T20:00')
      await expect(userPage.getByLabel('餐次', { exact: true })).toHaveValue('breakfast')
      const editedResponse = userPage.waitForResponse((response) => response.request().method() === 'PATCH' && response.url().includes('/meal-records/'))
      await userPage.getByRole('button', { name: '保存修改' }).click()
      const edited = await editedResponse
      expect(edited.status()).toBe(200)
      expect(edited.request().postDataJSON()).toMatchObject({ meal_slot: 'breakfast', time_zone: timeZone })
      await expect(userPage.getByRole('heading', { name: '餐食详情' })).toBeVisible()
      await expect(userPage.getByText(/早餐 ·/)).toBeVisible()
      await userPage.getByRole('link', { name: '编辑餐食记录' }).click()
      await userPage.getByLabel('餐次', { exact: true }).selectOption('snack')
      await userPage.getByRole('button', { name: '保存修改' }).click()
      await expect(userPage.getByText(/加餐 ·/)).toBeVisible()
