...
test('specific recipe confirmation survives reload and rejects an administratively disabled choice', async ({ page, request }) => {
  ...
  const breakfast = page.getByRole('heading', { name: '早餐', exact: true }).locator('xpath=ancestor::*[@data-slot="card"][1]')
  const dinner = page.getByRole('heading', { name: '晚餐', exact: true }).locator('xpath=ancestor::*[@data-slot="card"][1]')
  ...
  await expect(page.getByRole('heading', { name: '午餐', exact: true }).locator('xpath=ancestor::*[@data-slot="card"][1]')).toContainText('验收清蒸鱼')
