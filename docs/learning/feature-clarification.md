...
用户端提交分析时带 `Prefer: respond-async`。后端先保存线程和运行记录并立即返回，执行任务由应用生命周期持有；浏览器随后连接 SSE，服务端持续读取已落库的安全事件，直到等待输入、完成或失败。旧客户端不带该请求头时仍可等待同步结果。每次执行领取唯一 PostgreSQL 租约，结束后按持有者精确释放；进程异常退出时由租约过期允许其他实例恢复。
