...
| 12 | [生成一日餐单](feature-daily-planning.md) | 如何按目标比较候选组合，并检查忌口和全天约束？ | 已更新午晚餐组合明细、逐项受限份量、分批预算与历史快照 |
...
| 18 | [菜谱候选池管理](feature-recipe-pool.md) | 如何设置多餐次、三维分类并参与配餐？ | 包含组合筛选、导入、分类编辑、审计与候选筛选边界 |
...
| 23 | [后端运行日志与请求关联](feature-operational-logging.md) | 如何按请求编号排错，避免打印敏感数据？ | 标准输出与安全关联 |

指定菜谱与份量的本地交互证据见[菜谱选择验收](../verification/2026-09-12-recipe-selection.md)。

## 后续维护规则

新增用户能力、改变后端关键链路或原文已不再成立时，更新受影响的功能文档；新功能才新增独立文档，并同步上方导航。局部重构、样式和机械修复无需新增教程。具体要求见[项目规范](../../AGENTS.md)，业务场景见[产品说明](../product.md)。
