...
PostgreSQL 中已发布且当前合格的目录是业务真相；`pg_trgm` 和 pgvector 是可重建派生检索层。唯一精确 canonical name/alias 命中可直接 `PASS`；其余文本或语义候选最多三个且一律 `ASK`。因此“米饭”绝不被“蛋炒饭”覆盖，`西红柿炒鸡蛋 → 番茄炒蛋` 等变体只能作为待确认候选。
