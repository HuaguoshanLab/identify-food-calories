...
管理员维护可用于餐单的成品菜候选，指定关联营养目录、单份重量、适用餐次（多选）、三维分类和口味标签。规划服务从这些候选里组合餐单。

### 后台列表筛选（2026-09-17）

候选较多时，可组合菜名关键词、适用餐次和状态，点击“筛选”查询；“重置”恢复全部。关键词是目录菜名的包含匹配，餐次是候选适用餐次数组包含该值，多个条件同时满足才会返回。

流程：筛选表单 → `admin-frontend/src/features/recipes/api/index.ts` 校验并编码查询参数 → 公开后台 API → `AdminService.list_recipe_candidates` 校验管理员权限 → `AdminRepository.list_recipe_candidates` 在数据库筛选、计数和分页 → 页面展示。`RecipeListPage` 应用条件后回到第一页并清空旧选择；全选批次和导出使用同一组已应用条件，下载模板不带筛选。仅修改输入但未点击筛选，不会改变操作范围。

验证：后台前端 77 项测试和类型检查通过，新增组件测试覆盖组合参数、分页与选择重置、筛选范围全选。真实浏览器访问被登录页阻挡，筛选交互与实际导出下载尚未完成跨栈验收；本次未运行 Playwright。

### 多餐次候选（2026-09-17）

同一份米饭可以同时用于午餐和晚餐，无需复制候选。后台编辑勾选多个餐次，或在 CSV 的“餐次”填写 `午餐|晚餐`；单餐次旧文件仍可导入。餐次可选早餐、午餐、晚餐、加餐，至少选一个，不允许重复。

流程：管理员编辑或导入 → Schema 校验并规范餐次顺序 → Service 保存和审计 → Repository 按当前餐次是否在数组中筛选 → 为规划器投影为当前餐次的候选 → 原有营养计算、组合与归档。迁移 0032 将旧餐次转为单元素数组，不重写历史。

关键代码：`backend/app/admin/schemas.py` 的 `RecipeMealSlots` 负责运行时校验；`backend/app/admin/recipe_csv.py` 负责分隔值；`backend/app/planning/repository.py` 使用 `meal_slots.any(...)` 查询数组是否包含指定餐次。数据库候选存多餐次，规划 DTO 的 `meal_slot` 仍表示本次选入的餐次，两者职责不同。同一天仍保留候选去重规则，适用午晚餐不代表同一候选可以在当天重复使用。

验证：CSV、分类编辑单测覆盖多选、重复值、导入重叠与审计；真实 PostgreSQL 测试覆盖同一候选进入午晚餐池和迁移往返。开发环境已通过页面将白米饭、小米饭、糙米饭设为午晚餐，并生成午餐白米饭组合、晚餐糙米饭组合。此次不代表真实模型或长期记忆服务效果验收。
