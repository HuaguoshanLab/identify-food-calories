...
[backend/app/planning/selection.py](../../backend/app/planning/selection.py) 使用 `planning-selection.v3`。输入是已经计算好的候选餐次；输出是一组三餐或空结果。

- 候选总量超过 1024 时停止，防止大量营养查询；该上限与每餐最多 12 个选项、最多 12³ 个组合的搜索上限分别控制。
