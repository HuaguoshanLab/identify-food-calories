...
每个候选先按目录和原有份量重算营养；对单独候选，若启用份量适配且有目标，再计算一个受限的新克数选项，重新调用营养工具。组合餐则只对角色池保留的条目，通过回调重算一套逐项份量。原份量和调整份量一起交给 `planning/selection.py` 的 `select_meals`，营养公式不变。

### 4.3 在有界组合中比较目标与偏好

[backend/app/planning/selection.py](../../backend/app/planning/selection.py) 使用 `planning-selection.v10`。输入是已经计算好的候选餐次；输出是一组三餐或空结果。
...
代码入口为 [bundles.py](../../backend/app/planning/bundles.py) 的 `BundlePool`。角色热量权重默认 45/35/20，仅帮助筛选，不是营养建议。每种组合保留原份量，并可新增一套受限调整份量；整餐的重量及四项营养是 `Decimal` 逐项求和，`PlannedMeal` 和 `PlanMealSnapshot` 的运行时校验会拒绝总计不一致的明细。组合无额外模型调用；同餐同菜的调整结果缓存，每餐最多增加 3K 次营养计算。原份量与调整份量交错尝试，共享次数和时间上限；调整失败仍可选原组合。

`PlannedMeal.source_recipe_ids` 返回组成菜谱的真实 ID，跨餐重复检查与近期重复比较使用这些 ID，避免把不同组合的外壳当成完全不同食物。用于整餐替换的组合 ID 则由餐次、各组成菜谱 ID 和修订号确定；标识不包含克数，因此换餐会排除原组合的全部份量版本，不能只调整克数冒充换菜。它不是新的目录条目。公开卡片不包含这些内部标识。
...
> 语法小注：`product` 的三组输入代表三个角色，最多产生 K³ 种食物搭配，每种最多两套份量，共享餐内预算；`uuid5` 为同一组来源生成稳定标识；`items` 默认空元组让旧餐单仍能通过校验。配置及回退见[后端组合预算](../../backend/README.md#午晚餐组合预算)。

### 4.4 先尝试受限份量，再检查目标偏差

`PlanningService._slot_energy` 先从全天热量目标中点扣除固定餐次和加餐，再按剩余餐次权重分配。`_portion_for_energy` 以分配热量除以原菜谱热量，得到份量倍率，夹在配置范围内后取整数克。组合餐先按角色权重拆分餐次热量，再逐项调用同一逻辑。默认范围是原克数的 75%～125%，是产品搜索限制，不是营养处方。
...
`SelectionCandidate.identity` 使用菜谱 ID、总克数和各项克数，让同菜不同份量及总重相同但组成不同的选项可以进入候选池；最终仍按菜谱 ID 禁止跨餐重复。每条菜谱最多新增一个选项、每餐最终最多 12 项，组合预算不扩大。这是有预算的近似搜索，不保证找到所有可行份量。
...
### 组合餐逐项份量适配验证（2026-09-16）

- `tests/planning/test_bundle_portion_adaptation.py` 覆盖逐项营养重算、原数据不变、角色池缓存、次数与时间上限、失败保留原组合、同总重不同组成、固定餐次与加餐、排除原组合全部份量，以及实际克数归档。规划测试 204 项、Agent/API 合约及运行配置测试 48 项通过；隔离 PostgreSQL 规划 API 测试 8 项通过。不同轮次的历史数字不应累加为独立用例总数。
- Ruff、四个改动源文件的定向 Mypy（依赖静默）、H5 类型检查通过。
- Playwright 组合餐端到端场景通过：固定份量超出目标时选择受限份量，逐项克数边界及四项营养重算一致，连续同文换餐保留其他餐次，停用候选后历史不变、重新生成失败保留旧计划。测试使用隔离数据库、合成目录及 Fake Provider；测试脚本 ESLint 通过。跟踪服务未启动，测试日志出现遥测导出失败，未验证遥测接收。
- 真实开发页面 `127.0.0.1:5178/app/plans` 重新生成第 3 版：晚餐米饭 174g、龙井虾仁 188g、问政山笋 188g，全天 1481.8 kcal，页面显示全部指标在目标范围内。再提交“晚餐换一份”保存第 4 版，主食改为馒头 113g，早餐与午餐文本逐字不变。历史第 3 版保留原组合；第 2 版仍保留米饭 200g、银鱼炒蛋 200g、山笋 250g 的固定份量午餐。
- 真实目录只读试算在 1400/1600/1800/2000/2200 kcal 五档目标全部通过最终校验，午晚餐均选到组合。试算未设置偏好或近期历史，不能作为成功率指标，也不能证明真实来源营养准确。真实页面受个人偏好与历史影响，仍可能选择单独候选。
- 自动化使用 Fake Provider；真实页面验收不构成真实模型或 Mem0 的效果评测。目录仍有份量与来源质量待复核项；受限搜索仍可能错过可行组合。
