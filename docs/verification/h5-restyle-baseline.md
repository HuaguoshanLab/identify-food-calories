...
范围：第 1 项「冻结现有功能基线」。只记录源码、测试和设计冲突，不修改运行时代码，不代表已经完成视觉改版。开始时 `docs/ui/h5-foundation.md` 与其 README 已有未提交修改，予以保留。
...
- 每完成一项停止，用户检查后再继续。后续功能实施仍遵循 GSD 讨论、计划、执行与验收流程。
...
第 1 项完成后停止。基线含现存 lint 问题和测试不稳定记录，不宣称全部门禁通过，不在本项顺手修复。

用户检查后，下一项为「全局视觉样式」：按设计稿统一主题 token、字体、圆角、边框和阴影；不会同时改四页业务流程。开始前按 GSD 完成相应范围与计划，并保留本基线的冲突优先级。
