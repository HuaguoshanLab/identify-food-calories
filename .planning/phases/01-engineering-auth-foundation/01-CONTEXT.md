...
- **D-29:** 仓库根目录必须维护 `README.md` 与 `AGENTS.md`；`frontend/`、`backend/` 也分别维护自己的 `README.md` 与 `AGENTS.md`，子级规则只能细化、不能放宽根级规则。
- **D-30:** 新增任何目录时，必须在同一次提交新增该目录的 `README.md`，并写明目录职责、允许依赖和当前文件索引；目录文件变化时同步更新索引。

### 邮箱验证、公开入口与后台项目

- **D-31:** 注册使用独立三步流程：`/register` 提交邮箱与密码，`/register/verify` 输入 6 位邮箱验证码，验证成功后账号激活并进入登录流程。
- **D-32:** 验证码只保存摘要，10 分钟过期、最多尝试 5 次、60 秒后可重发；新码使旧码失效，验证成功后立即作废。发送与重发接口使用统一响应，避免邮箱枚举。
- **D-33:** 本地开发通过 Docker Mailpit 捕获邮件；生产邮件通过可替换 `MailProvider` 发送，不在 Phase 1 锁定云供应商。
- **D-34:** 用户无需登录即可访问产品介绍、登录、注册、隐私与条款页面；拍照分析、记录、规划和个人资料等核心 H5 功能必须登录。受保护深链登录成功后返回原地址，Phase 1 不实现游客分析。
- **D-35:** 管理后台最终使用独立 `admin-frontend/` React 项目，与 `frontend/`、`backend/` 同级并独立构建部署；Phase 1 不创建空后台项目，也不在用户 H5 中实现 `/admin` 页面，只交付后端 admin probe、RBAC 与管理员 CLI，完整后台在 Phase 6。
