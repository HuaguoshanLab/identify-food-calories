...
- 执行阶段同步更新现有功能文档；新增功能才新增独立文档。验收检查总目录链接、真实代码位置、精简代码逻辑以及功能概述、核心能力、业务背景、流程、语法说明和验证方法。
- 教学内容侧重 AI 与后端，具体要求遵循根目录 `AGENTS.md` 的 Teaching Contract 和 `PROJECT.md` 的 Learning Contract。
- 已完成阶段的计划、总结保留历史记录；恢复旧计划时，将旧教学路径映射到当前功能文档，不重新生成已归档的旧教程。`docs/after/` 只存历史资料。
...
- [x] **Phase 6: 用户看板与后台管理** — 完成趋势分析、周复盘、营养目录管理、模型配置、运行审计与 RBAC 管理界面。（36/36 计划完成，阶段验证 5/5 通过） (completed 2026-09-05)
...
- [x] 06-30-PLAN.md — 服务端用户统计时区窗口与 weekly API 边界

**Wave 19** *(blocked on Wave 18 completion)*

- [x] 06-31-PLAN.md — H5 时区确认、local Monday 序列化与组件回归

**Wave 20** *(blocked on Wave 19 completion)*

- [x] 06-32-PLAN.md — 双 Records E2E、真实浏览器与文档时区证据

**Wave 21** *(gap closure; blocked on 06-32)*

- [x] 06-33-PLAN.md — Records 时区确认幂等、竞争与非法输入安全

**Wave 22** *(blocked on Wave 21 completion)*

- [x] 06-34-PLAN.md — Dashboard 服务端当前窗口与历史周边界

**Wave 23** *(blocked on Wave 22 completion)*

- [x] 06-35-PLAN.md — H5 当前窗口读取与同/异时区确认语义

**Wave 24** *(blocked on Wave 23 completion)*

- [x] 06-36-PLAN.md — 真实跨时区 E2E、浏览器验收与证据
...
- [x] 06.3-02-PLAN.md — 建立严格 1024 维 Embedding DTO、Protocol 与离线 Fake
- [x] 06.3-03-PLAN.md — 接入 DashScope Adapter、Provider 配置与安全工厂
...
- [x] 06.3-14-PLAN.md — 实现双图一致 ASK、clarification、resume 与 parity
...
- [x] 06.3-20-PLAN.md — 完成中文教学、真实 Playwright 门禁和内置浏览器验收
...
**Requirements**: AUTH-04, ADM-01, ADM-05, UI-03, ARC-08
**Depends on:** Phase 6
**Plans:** 2 plans

Plans:
- [x] 06.2-01-PLAN.md — 账号查询、固定角色、原子升降权、幂等审计与实时 RBAC
- [x] 06.2-02-PLAN.md — 系统管理导航、管理员/角色页面、跨栈验收与教学文档

**Success Criteria:**

1. 后台新增“系统管理”一级菜单，以及“管理员管理”和“角色管理”两个二级页面。
2. 管理员可分页查看全部账号及邮箱，按邮箱、状态和角色筛选；响应不包含会话、健康或餐食数据。
3. 已验证且有效的普通账号可被晋升；其他管理员可被降权，但自己和最后一个有效管理员不可被降权。
4. 角色变更在同一事务内写入不可变审计，降权立即阻断后台访问但保留普通用户会话。
5. 固定 `user/admin` 角色权限说明、组件/API/真实 PostgreSQL/Playwright 测试与内置浏览器验收全部通过。
...
Plans:
- [x] 06.1-01-PLAN.md — 候选持久化、目录资格与可选加餐领域合同
- [x] 06.1-02-PLAN.md — 后台候选 CSV、批量生命周期与审计 API
- [x] 06.1-03-PLAN.md — 候选轮换、规划图、计划归档和加餐调整
- [x] 06.1-04-PLAN.md — 后台菜谱管理与用户端可选加餐界面
- [x] 06.1-05-PLAN.md — 跨栈验收、教学文档和浏览器验证
...
| 2. 可追问的 Agent 核心 | 18/18 | Accepted; release FAIL | 2026-08-31 |
| 3. 多模态餐食分析闭环 | 5/5 | Complete   | 2026-09-01 |
| 4. 餐食记录与长期记忆 | 7/7 | Complete    | 2026-09-01 |
| 5. 饮食规划子图 | 11/11 | Complete   | 2026-09-02 |
| 6. 用户看板与后台管理 | 36/36 | Complete    | 2026-09-05 |
| 06.1. 菜谱管理与餐单候选池 | 5/5 | Complete | 2026-09-08 |
| 06.2. 系统管理与角色治理 | 2/2 | Complete; verification passed | 2026-09-09 |
| 06.3. 混合菜品检索 | 21/21 | Complete; verification passed | 2026-09-12 |
